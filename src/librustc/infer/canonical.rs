...
use ty::{self, CanonicalVar, Lift, Region, Ty, TyCtxt, TypeFlags};
use ty::subst::Kind;
...
impl<'cx, 'gcx, 'tcx> InferCtxt<'cx, 'gcx, 'tcx> {
    ...
    /// values from the canonical query. It's easiest to explain what
    /// is happening here by example.
    ///
    /// Imagine we start out with the query `?A: Foo<'static, ?B>`. We would
    /// canonicalize that by introducing two variables:
    ///
    ///     ?0: Foo<'?1, ?2>
    ///
    ...
    pub fn instantiate_query_result<R>(
        ...
        query_result: &Canonical<QueryResult<'tcx, R>>,
    ) -> InferResult<'tcx, R>
    where
        R: Debug + TypeFoldable<'tcx>,
    {
        ...
        let result_subst = &CanonicalVarValues {
            var_values: query_result
                .variables
                .iter_enumerated()
                .map(|(index, info)| match opt_values[index] {
                    Some(k) => k,
    ...
    pub fn unify_canonical_vars(
        ...
        self.commit_if_ok(|_| {
            ...
            for (value1, value2) in variables1.var_values.iter().zip(&variables2.var_values) {
                if let (Some(v1), Some(v2)) = (value1.as_type(), value2.as_type()) {
                    obligations.extend(self.at(cause, param_env).eq(v1, v2)?.into_obligations());
                } else if let (Some(v1), Some(v2)) = (value1.as_region(), value2.as_region()) {
                    if let (ty::ReErased, ty::ReErased) = (v1, v2) {
                        // no action needed
                    } else {
                        obligations
...
impl<'cx, 'gcx, 'tcx> TypeFolder<'gcx, 'tcx> for CanonicalVarValuesSubst<'cx, 'gcx, 'tcx> {
    ...
    fn fold_ty(&mut self, t: Ty<'tcx>) -> Ty<'tcx> {
        match t.sty {
            ty::TyInfer(ty::InferTy::CanonicalTy(c)) => {
                self.var_values.var_values[c].as_type().unwrap_or_else(|| {
                    bug!(
                        "{:?} is a type but value is {:?}",
                        c,
                        self.var_values.var_values[c]
                    )
                })
            }
    ...
    fn fold_region(&mut self, r: ty::Region<'tcx>) -> ty::Region<'tcx> {
        match r {
            ty::RegionKind::ReCanonical(c) => self.var_values.var_values[*c]
                .as_region()
                .unwrap_or_else(|| {
                    bug!(
                        "{:?} is a region but value is {:?}",
                        c,
                        self.var_values.var_values[*c]
                    )
                }),
...
BraceStructTypeFoldableImpl! {
    impl<'tcx, C> TypeFoldable<'tcx> for Canonical<C> {
        variables,
        value,
...
BraceStructTypeFoldableImpl! {
    impl<'tcx, R> TypeFoldable<'tcx> for QueryResult<'tcx, R> {
        var_values, region_constraints, value

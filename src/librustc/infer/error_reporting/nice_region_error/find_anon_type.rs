...
impl<'a, 'gcx, 'tcx> NiceRegionError<'a, 'gcx, 'tcx> {
    ...
    fn find_component_for_bound_region(
        ...
        let mut nested_visitor = FindNestedTypeVisitor {
            ...
            bound_region: *br,
            found_type: None,
            debruijn_index: ty::DebruijnIndex::INNERMOST,
        };
...
impl<'a, 'gcx, 'tcx> Visitor<'gcx> for FindNestedTypeVisitor<'a, 'gcx, 'tcx> {
    ...
    fn visit_ty(&mut self, arg: &'gcx hir::Ty) {
        match arg.node {
            hir::TyBareFn(_) => {
                self.debruijn_index.shift_in(1);
                intravisit::walk_ty(self, arg);
                self.debruijn_index.shift_out(1);
                return;
            }
            ...
            hir::TyRptr(ref lifetime, _) => {
                ...
                match (self.tcx.named_region(hir_id), self.bound_region) {
                    ...
                    (
                        ...
                        debug!(
                            "LateBoundAnon index = {:?} anon_index = {:?} br_index={:?}",
                            debruijn_index.index(),
                            anon_index,
                            br_index
                        );
                        if debruijn_index == self.debruijn_index && anon_index == br_index {
                    ...
                    (
                        ...
                        debug!(
                            "FindNestedTypeVisitor::visit_ty: LateBound index = {:?}",
                            debruijn_index
                        );
                        debug!("self.infcx.tcx.hir.local_def_id(id)={:?}", id);
                        debug!("def_id={:?}", def_id);
                        if debruijn_index == self.debruijn_index && id == def_id {
                            self.found_type = Some(arg);
                            return; // we can stop visiting now
                        }
                    }

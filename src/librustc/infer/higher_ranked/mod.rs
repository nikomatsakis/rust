...
// file at5B the top-level directory of this distribution and at
...
use super::{CombinedSnapshot, InferCtxt, LateBoundRegion, HigherRankedType,
            SkolemizationMap, SubregionOrigin};
...
pub trait HigherRankedRelations<'a,'tcx> {
    ...
    fn higher_ranked_match<T, U>(&self,
                                 span: Span,
                                 a: &Binder<(T, U)>,
                                 b: &T)
...
impl<'a,'tcx> HigherRankedRelations<'a,'tcx> for CombineFields<'a,'tcx> {
        ...
        where T: Relate<'a,'tcx>
    {
        ...
        return self.infcx.commit_if_ok(|snapshot| {
            ...
            match leak_check(self.infcx, span, &skol_map, snapshot) {
                Ok(()) => { }
            ...
            // them off.
            pop_skolemized(self.infcx, skol_map, snapshot);
...
impl<'a,'tcx> InferCtxtExt for InferCtxt<'a,'tcx> {
    fn tainted_regions(&self,
                       ...
                       directions: TaintDirections)
                       -> FnvHashSet<ty::Region> {
        self.region_vars.tainted(&snapshot.region_vars_snapshot,
                                 region,
                                 directions)
    }
    ...
    where T : TypeFoldable<'tcx>
{
    let (result, map) = infcx.tcx.replace_late_bound_regions(binder, |br| {
        infcx.region_vars.push_skolemized(br, &snapshot.region_vars_snapshot)
    });
...
{
    debug!("leak_check(skol_map={:?}, snapshot={:?})",
           skol_map, snapshot.region_vars_snapshot);
    ...
    for (&skol_br, &skol) in skol_map {
        ...
        let incoming_taints = infcx.tainted_regions(snapshot,
                                                    skol,
        ...
        for &tainted_region in &incoming_taints {
            ...
            match tainted_region {
                ty::ReVar(vid) => {
                    if new_vars.contains(&vid) { continue; }
                }
                _ => {
    ...
    for (_, &skol) in skol_map {
        ...
        let outgoing_taints = infcx.tainted_regions(snapshot,
                                                    skol,
        ...
        for &tainted_region in &outgoing_taints {
            match tainted_region {
                ...
                _ => {
                    infcx.region_vars.make_subregion(
    ...
    where T : TypeFoldable<'tcx>
{
    ...
    let inv_skol_map: FnvHashMap<ty::Region, ty::BoundRegion> =
        ...
        .flat_map(|(&skol_br, &skol)| {
            infcx.tainted_regions(snapshot, skol, TaintDirections::incoming())
                 .into_iter()
    ...
    pop_skolemized(infcx, skol_map, snapshot);
...
pub fn pop_skolemized<'a,'tcx>(infcx: &InferCtxt<'a,'tcx>,
                               skol_map: SkolemizationMap,
                               snapshot: &CombinedSnapshot)
{
    debug!("pop_skolemized({:?})", skol_map);
    let skol_regions: FnvHashSet<_> = skol_map.values().cloned().collect();
    infcx.region_vars.pop_skolemized(&skol_regions, &snapshot.region_vars_snapshot);
}

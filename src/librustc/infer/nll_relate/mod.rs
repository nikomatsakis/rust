...
use crate::ty::{self, CanonicalVar, Ty, TyCtxt};
...
/// The `TypeRelating` type is a `TypeRelation`, meaning that you can
/// use the `relate_tys` method to impose a relationship between two
/// types.
///
/// The form of this relationship is controlled by the
/// `ambient_variance` -- e.g., if the ambient variance is
/// `Covariant`, and you invoke `relate_tys(a, b)`, then we will
/// generate the constraints needed for `a <: b`. Similarly, if
/// `Invariant`, we will generate the constraints needed for `a == b`,
/// etc.
///
/// The constraints are reported by invoking methods on the
/// `delegate`, which is defined by a trait (`TypeRelatingDelegate`).
///
/// As noted on the module doc comment, we generally assume that the
/// types in question do not have type inference variables. However,
/// the type in `a` **is** permitted to contain canonical variables --
/// these arise when enforcing user-given type annotations in NLL. In
/// that case, a non-empty `canonical_var_infos` should be given
/// describing the universe etc for each such variable. You can then
/// use `into_canonical_var_values` to extract the resulting values
/// out.
    ...
    D: TypeRelatingDelegate<'tcx>,
{
    ...
    /// Extract out the values that we inferred for any canonical values.
    ///
    /// Note that canonical values must always appear in the `a` value!
    pub fn into_canonical_var_values(self) -> IndexVec<CanonicalVar, Option<Kind<'tcx>>> {
        self.canonical_var_values
    }

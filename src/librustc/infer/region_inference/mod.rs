        ...
        while prev_len < self.len() {
            ...
            for undo_entry in undo_log {
                match undo_entry {
                    ...
                    &AddVerify(i) => {
                        panic!("we do not add verifys \
                                during skolemization: {:?}", verifys[i]);
                    }
                    &Purged |
                    &AddCombination(..) |
                    &AddVar(..) |
...
impl<'a, 'tcx> RegionVarBindings<'a, 'tcx> {
    ...
    pub fn pop_skolemized(&self,
        ...
        debug!("pop_skolemized_regions(skols={:?}, snapshot={:?})",
               skols, snapshot);
        ...
        let mut undo_log = self.undo_log.borrow_mut();
        let undo_log = &mut undo_log[snapshot.length..];

        let constraints_to_kill: Vec<usize> = {
            let verifys = self.verifys.borrow();
            undo_log.iter()
                    .enumerate()
                    .rev()
                    .filter(|&(_, undo_entry)| kill_constraint(skols,
                                                               undo_entry,
                                                               &verifys[..]))
                    .map(|(index, _)| index)
                    .collect()
        ...
        fn kill_constraint(skols: &FnvHashSet<ty::Region>,
            ...
            match undo_entry {
                ...
                &AddGiven(_, _) =>
                    false,
                &AddVerify(i) =>
                    panic!("we never mix generic bounds \
                            and skolemization: {:?}", verifys[i]),

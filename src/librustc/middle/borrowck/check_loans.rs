...
impl<'a> CheckLoanCtxt<'a> {
    ...
    pub fn report_error_if_loan_conflicts_with_restriction(&self,
        ...
        let illegal_if = match loan2.kind {
            ty::MutBorrow => RESTR_ALIAS | RESTR_MUTATE,
            ty::ImmBorrow => RESTR_ALIAS | RESTR_FREEZE,
            ty::UniqueImmBorrow => RESTR_ALIAS | RESTR_FREEZE,
        };
    ...
    pub fn check_assignment(&self, expr: &ast::Expr) {
        ...
        fn check_for_aliasability_violation(this: &CheckLoanCtxt,
            ...
            loop {
                match cmt.cat {
                    mc::cat_deref(b, _, mc::BorrowedPtr(ty::MutBorrow, _)) |
                    mc::cat_deref(b, _, mc::BorrowedPtr(ty::UniqueImmBorrow, _)) |
                    mc::cat_downcast(b) |
                    mc::cat_deref(b, _, mc::OwnedPtr) |
                    mc::cat_interior(b, _) |
                    mc::cat_discr(b, _) => {
                    ...
                    mc::cat_static_item(..) |
                    mc::cat_deref(_, _, mc::GcPtr) |
                    mc::cat_deref(_, _, mc::BorrowedPtr(ty::ImmBorrow, _)) => {

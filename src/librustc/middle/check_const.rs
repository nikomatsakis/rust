        ...
        match self.rvalue_borrows.remove(&ex.id) {
            Some(ast::MutImmutable) => {
                ...
                if !self.qualif.intersects(NON_STATIC_BORROWS) {
                    ...
                    self.qualif(HAS_STATIC_BORROWS);
                }
            }
            Some(ast::MutMutable) => {
...
fn check_expr<'a, 'tcx>(v: &mut CheckCrateVisitor<'a, 'tcx>,
    ...
    match e.node {
        ...
        ast::ExprPath(_) | ast::ExprQPath(_) => {
            ...
            match def {
                ...
                Some(def::DefConst(did)) => {
                    let const_expr = if did.krate == ast::LOCAL_CRATE {
                        let const_item = v.tcx.map.expect_item(did.node);
                        ...
                        const_eval::lookup_const_by_id(v.tcx, did)
                    };
                    let inner = match const_expr {
                        Some(expr) => v.global_expr(Mode::Const, expr),
                        None => NOT_CONST
                    };
                    v.qualif(inner);
                }
        ...
        ast::ExprBlock(ref block) => {
            ...
            let mut block_span_err = |&mut: span| {
                v.qualif(NOT_CONST);
                if v.mode != Mode::Var {
        ...
        ast::ExprStruct(..) => {
            ...
            if did == v.tcx.lang_items.unsafe_type() {
                v.qualif(MUTABLE_MEM);
            }
        }

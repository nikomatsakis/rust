...
impl<TYPER:Typer> MemCategorizationContext<TYPER> {
            ...
            Some(adjustment) => {
                match *adjustment {
                    ...
                    ty::AutoAddEnv(..) => {
                        ...
                        // Result is an rvalue.
                        let expr_ty = if_ok!(self.expr_ty_adjusted(expr));
                        Ok(self.cat_rvalue_node(expr, expr_ty))
                    }
        ...
        match expr.node {
          ast::ExprUnary(_, ast::UnDeref, e_base) => {
            if self.typer.is_method_call(expr.id) {
                return Ok(self.cat_rvalue_node(expr, expr_ty));
            }
    ...
    pub fn cat_rvalue_node<N:ast_node>(&mut self,
                                       node: &N,
                                       expr_ty: ty::t) -> cmt {
        match self.typer.temporary_scope(node.id()) {
            Some(scope) => {
        ...
        match pat.node {
                    ...
                    for (i, &subpat) in subpats.iter().enumerate() {
                        ...
                        let cmt_field =
                            self.cat_imm_interior(
                                pat, cmt, subpat_ty,
                                InteriorField(PositionalField(i)));
                        self.cat_pattern(cmt_field, subpat, |x,y,z| op(x,y,z));
                    }
                }
                Some(&ast::DefStatic(..)) => {
                    for &subpat in subpats.iter() {
                        self.cat_pattern(cmt, subpat, |x,y,z| op(x,y,z));
                    }
                }
                _ => {
                    self.tcx().sess.span_bug(
          ...
          ast::PatStruct(_, ref field_pats, _) => {
            ...
            for fp in field_pats.iter() {
                ...
                self.cat_pattern(cmt_field, fp.pat, |x,y,z| op(x,y,z));
            }
          }
          ...
          ast::PatVec(ref before, slice, ref after) => {
              ...
              for &before_pat in before.iter() {
                  self.cat_pattern(elt_cmt, before_pat, |x,y,z| op(x,y,z));
              }
              ...
              for &after_pat in after.iter() {
                  self.cat_pattern(elt_cmt, after_pat, |x,y,z| op(x,y,z));
              }
          }
...
impl cmt_ {
    ...
    pub fn freely_aliasable(&self) -> Option<AliasableReason> {
        ...
        match self.cat {
            ...
            cat_arg(_) |
            cat_upvar(..) |
            cat_deref(_, _, UnsafePtr(..)) | // of course it is aliasable, but...
            cat_deref(_, _, BorrowedPtr(ty::MutBorrow, _)) |
            cat_deref(_, _, BorrowedPtr(ty::UniqueImmBorrow, _)) => {
                None
            }

                                         ...
                                         Vec<PredicateObligation<'tcx>>)
{
    ...
    let impl_generics = ty::lookup_item_type(selcx.tcx(), impl_def_id).generics;
    let bounds = impl_generics.to_bounds(selcx.tcx(), impl_substs);
    let Normalized { value: bounds, obligations: normalization_obligations2 } =
        project::normalize(selcx, ObligationCause::dummy(), &bounds);
    let impl_obligations =
        util::predicates_for_generics(selcx.tcx(), ObligationCause::dummy(), 0, &bounds);

...
```rust
trait Foo<A> { fn dummy(&self, arg: A) { } }
```
...
represents skolemized region #0). Note that now have no quantifies; in
terms of the compiler type, this changes from a `ty::PolyTraitRef` to
a `TraitRef`. We would then create the `TraitRef` from the impl, using
fresh variables for it's bound regions (and thus getting `Foo<&'$a
int>`, where `'$a` is the inference variable for `'a`). Next we relate
the two trait refs, yielding a graph with the constraint that `'0 ==
'$a`. Finally, we check for skolemization leaks, basically by
searching from the skolemized region to find the set of regions that
it is related to in any way. That set must consistent *solely* of
itself and region variables from the impl. This is called the "taint"
set.  If the taint set includes any other region, then the match is a
failure. In this case, it will succeed.
...
We would then see whether we can match the impl regions
against this. This succeeds with the substitution `'a => '0`. Finally,
we check for skolemization "leaks" -- a leak is basically any attempt
to relate a skolemized region to another skolemized region, or to any
region that pre-existed the impl match. In other words, it is ok to
relate `'a` and `'0`, but if we had an impl like this:

    impl Foo<&'static int> for MyStruct { }

we would have created an obligation that `'static == '0`, which should
yield an error.
...
leak check passed, so this taint set consistently solely of the
skolemized region itself plus various intermediate region
variables. We then walk the trait-reference and convert every region
in that taint set back to a late-bound region, so in this case we'd
wind up with `for<'a> F : Bar<&'a int>`.

...
impl<'a> TypeFolder for ItemSubstFolder<'a> {
                          ...
                          -> traits::VtableOrigin
    {
        match origin.vtable {
            traits::VtableImpl(..) |
            traits::VtableUnboxedClosure(..) |
            traits::VtableBuiltin => {
                ty_fold::super_fold_vtable_origin(self, origin)
            }
            traits::VtableParam(ref param) => {
...
impl ResolveVtablePath for subst::ItemSubsts {
                           ...
                           -> traits::VtableOrigin
    {
        ...
        if path.supertraits.len() == 0 {
            ...
            trace_supertraits(tcx, &root_origin, &path.supertraits)
        }
    }
}

fn trace_supertraits<'a>(tcx: &ty::ctxt,
                         mut vtable: &'a traits::VtableOrigin,
                         ...
                         -> traits::VtableOrigin
{
    ...
    for &i in supertraits.iter() {
        let superorigin = {
            let v = assert_vtable_impl(tcx, vtable);
            v.nested.get(subst::SelfSpace, i)
        };
        vtable = superorigin;
    }
...
fn assert_vtable_impl<'a>(tcx: &ty::ctxt,
                          vtable_origin: &'a traits::VtableOrigin)
                          -> &'a traits::VtableImpl<traits::VtableOrigin>
{
    /*!
    ...
    match vtable_origin.vtable {
        ...
        traits::VtableParam(..) => {
            tcx.sess.bug(
                "trace_supertraits() encountered non-vtable-impl");
        }
    }
}

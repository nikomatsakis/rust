              ...
              -> ValueRef
{
    ...
    let vtable = match *origin {
        traits::Vtable(ref vtable, None) => vtable,
        ref r => {
            ccx.sess().bug(
                format!("bad vtable: {}", r.repr(bcx.tcx())).as_slice());
        }
    ...
    // Check the cache.
    let hash_id = (self_ty, monomorphize::make_vtable_id(ccx, vtable));

...
pub trait TypeFolder {
    ...
    fn fold_resolution<R:TypeFoldable>(&mut self, r: &traits::Resolution<R>) -> traits::Resolution<R> {
        super_fold_resolution(self, r)
    }
...
impl<R:TypeFoldable> TypeFoldable for traits::Resolution<R> {
    fn fold_with<F:TypeFolder>(&self, folder: &mut F) -> traits::Resolution<R> {
        folder.fold_resolution(self)
    }
}
...
pub fn super_fold_resolution<R:TypeFoldable,T:TypeFolder>(this: &mut T,
                                                          resolution: &traits::Resolution<R>)
                                                          -> traits::Resolution<R>
{
    match *resolution {
        traits::ResolvedTo(ref s) => traits::ResolvedTo(s.fold_with(this)),
        traits::ResolvedToUnimpl => traits::ResolvedToUnimpl,
        traits::ResolvedToOverflow => traits::ResolvedToOverflow
    }
}

pub fn super_fold_vtable_origin<T:TypeFolder>(
    this: &mut T,
    resolution: &traits::VtableOrigin)
    -> traits::VtableOrigin
{
    match *resolution {
        traits::Vtable(ref v, ref o) => traits::Vtable(v.fold_with(this), (*o).clone()),
        traits::VtableParam(ref p, ref o) => traits::VtableParam(p.fold_with(this), (*o).clone()),
        traits::Builtin => traits::Builtin
    }
}

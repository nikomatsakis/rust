...
pub fn lookup_method_for_for_loop(fcx: &FnCtxt,
    ...
    let method = method::lookup_in_traits(fcx,
                                          iterator_expr.span,
                                          &*iterator_expr,
    ...
    let method_type = match method {
        Ok(Some(ref method)) => method.ty,
        Ok(None) | Err(ErrorReported) => {
            fcx.tcx().sess.span_err(iterator_expr.span,
    ...
    match method {
        ...
        Ok(None) | Err(ErrorReported) => ty::mk_err()
    }
}

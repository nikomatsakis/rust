...
use middle::traits::{Obligation, Selection,
                     Vtable, VtableImpl, VtableUnboxedClosure,
                     VtableParam, VtableBuiltin};
use middle::ty;
...
{
    let method_callee =
        method_callee(fcx, call_span, selection, method_name,
                     ...
                     -> MethodCallee
{
    let (method_substs, method, method_origin) =
        instantiate_method_params(fcx, call_span, selection, method_name,
                                  supplied_method_type_params);
                             ...
                             -> (Substs, Rc<ty::Method>, MethodOrigin)
{
    let (substs, method, method_origin) =
        decompose_vtable_selection(fcx, call_span, selection, method_name);
...
fn decompose_vtable_selection(fcx: &FnCtxt,
                              call_span: Span,
                              vtable_selection: Vtable<Obligation>,
                              method_name: ast::Name)
                              -> (Substs, Rc<ty::Method>, MethodOrigin)
{
    match vtable_selection {
        VtableImpl(vtable) => {
            decompose_impl_vtable(fcx, vtable, method_name)
        }

        VtableUnboxedClosure(def_id) => {

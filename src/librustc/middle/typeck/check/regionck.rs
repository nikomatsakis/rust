...
categories, but we may adjust this later. The region check basically
just ignores the borrow kind (and derived mutabilities) that are
returned from mem-categorization, it just consults the upvar-id and
uses that to lookup and adjust the entry in the master table.
(Another option would be to use a unification scheme, where instead of
returning a concrete borrow kind like `ty::ImmBorrow`, we return a
...
fn link_local(rcx: &mut Rcx, local: &ast::Local) {
    ...
    let discr_cmt = if_ok!(mc.cat_expr(init_expr));
...
fn link_match(rcx: &mut Rcx, discr: &ast::Expr, arms: &[ast::Arm]) {
    ...
    let mut mc = mc::MemCategorizationContext { typer: rcx };
    let discr_cmt = if_ok!(mc.cat_expr(discr));
...
fn link_autoref(rcx: &mut Rcx,
    ...
    let mut mc = mc::MemCategorizationContext { typer: rcx };
    let expr_cmt = if_ok!(mc.cat_expr_autoderefd(expr, autoderefs));
...
fn link_by_ref(rcx: &mut Rcx,
    ...
    let mut mc = mc::MemCategorizationContext { typer: rcx };
    let expr_cmt = if_ok!(mc.cat_expr(expr));
...
fn link_region(rcx: &mut Rcx,
    ...
    loop {
        ...
        match cmt_borrowed.cat {
            mc::cat_deref(base, _, mc::BorrowedPtr(m, r_borrowed)) => {
                ...
                // FIXME what about UniqueImmBorrow
                if m == ty::MutBorrow {
                    // Reborrowing the contents of an `&mut`, either
                    // with `&mut` or `&`, requires that the `&mut` be
                    // located in a non-aliasable location. If the
                    // base is an upvar, then we may need to influence
                    // how the upvar is borrowed.
            ...
            mc::cat_deref(base, _, mc::BorrowedPtr(..)) => {
                match base.cat {
                    mc::cat_upvar(ref upvar_id, _) => {
                        ...
                        return adjust_upvar_borrow_kind(ub, ty::MutBorrow);
                    }
                ...
                match base.cat {
                    mc::cat_upvar(ref upvar_id, _) => {
                        ...
                        let mut upvar_borrow_map =
                            rcx.fcx.inh.upvar_borrow_map.borrow_mut();
                        let ub = upvar_borrow_map.get().get_mut(upvar_id);
                        return adjust_upvar_borrow_kind(ub, ty::UniqueImmBorrow);
                    }
...
fn link_upvar_borrow_kind(rcx: &mut Rcx,
    ...
    match upvar_borrow_map.get().find_mut(&outer_upvar_id) {
        Some(outer_borrow) => {
            adjust_upvar_borrow_kind(outer_borrow, inner_borrow.kind);
        }
        None => { /* outer closure is not a stack closure */ }
    }
}

fn adjust_upvar_borrow_kind_for_loan(upvar_borrow: &mut ty::UpvarBorrow,
                                     mutbl: ast::Mutability) {
    adjust_upvar_borrow_kind(upvar_borrow, ty::BorrowKind::from_mutbl(mutbl))
}

fn adjust_upvar_borrow_kind(upvar_borrow: &mut ty::UpvarBorrow,
                            kind: ty::BorrowKind) {
    /*!
    ...
    debug!("adjust_upvar_borrow_kind: upvar_borrow={:?} kind={:?}",
           upvar_borrow, kind);

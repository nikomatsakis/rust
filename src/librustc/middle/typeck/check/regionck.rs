...
Whenever there is a closure expression, we need to determine
how the
...
fn check_expr_fn_block(rcx: &mut Rcx,
    ...
    fn constrain_free_variables(rcx: &mut Rcx,
        ...
        for freevar in freevars.iter() {
            ...
            // FIXME This will eventually be unnecessary
...
fn link_pattern(mc: &mut mc::MemCategorizationContext<&mut Rcx>,
                discr_cmt: mc::cmt,
                root_pat: @ast::Pat) {
    mc.cat_pattern(discr_cmt, root_pat, |mc, sub_cmt, sub_pat| {
            match sub_pat.node {
                // `ref x` pattern

...
struct Wf<'a> {
    tcx: &'a ty::ctxt,
    unboxed_closure_types: Option<&'a DefIdMap<ty::ClosureTy>>,
...
pub fn region_wf_constraints(
    tcx: &ty::ctxt,
    unboxed_closure_types: Option<&DefIdMap<ty::ClosureTy>>,
    ty: ty::t,
    ...
    -> Vec<WfConstraint>
{
    ...
    // workaround fact that Option<T> is invariant:
    let unboxed_closure_types = unboxed_closure_types.map(|x| &*x);
...
impl<'a> Wf<'a> {
    fn accumulate_from_ty(&mut self, ty: ty::t) {
        ...
        match ty::get(ty).sty {
            ...
            ty::ty_unboxed_closure(ref def_id) => {
                let closure_ty = self.unboxed_closure_types
                                     .and_then(|c| c.find(def_id));
                match closure_ty {
                    None => {
                        self.tcx.sess.bug(
                            format!(
                                "No unboxed closure type found: {}",
                                def_id.repr(self.tcx)).as_slice());
                    }

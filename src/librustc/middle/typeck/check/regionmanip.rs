...
/**
 * Type folder which replaces regions bound on a type with free
 * regions associated with the given `body_id`. Used when checking
 * impls and method bodies. As an example, if you have an impl
 * like:
 *
 *     impl<'a> ... Foo<'a> { ... }
 *
 * then the self type will be `Foo<re_type_bound('a)>`. This folder
 * would convert that type to `Foo<re_free('a)>`. Function bound regions
 * are not affected.
 */
struct TypeBoundRegionSubstitutor {
    tcx: ty::ctxt,
    body_id: ast::NodeId,
}

impl TypeBoundRegionSubstitutor {
    pub fn new(tcx: ty::ctxt,
               body_id: ast::NodeId)
               -> TypeBoundRegionSubstitutor {
        TypeBoundRegionSubstitutor {
            tcx: tcx,
            body_id: body_id
        }
...
impl TypeFolder for TypeBoundRegionSubstitutor {
    fn tcx(&self) -> ty::ctxt {
        self.tcx
    }
    ...
    fn fold_region(&mut self, r: ty::Region) -> ty::Region {
        ...
        match r {
            ty::re_type_bound(param_id, index, ident) => {
                let def_id = ast_util::local_def(param_id);
                ty::re_free(ty::FreeRegion {
                        scope_id: self.body_id,
                        bound_region: ty::br_named(def_id, ident)})
            }
            _ => r
        }
    }

...
use middle::traits::{FulfillmentContext, Obligation,
                     obligations_for_generics};
                    ...
                    impl_trait_ref: &ty::TraitRef)
{
    ...
    debug!("obligations for impl {} are {}",
           impl_item.id,
           obligations.repr(tcx));

    let mut fulfillment_context = FulfillmentContext::new();

    let pending_resolutions =
        obligations.map_move(
            |o| fulfillment_context.register_obligation(tcx, o));

    match fulfillment_context.select_all_or_error(fcx.infcx(),
                                                  &fcx.inh.param_env) {
        Ok(()) => { }
        Err(e) => { report_fulfillment_error(fcx, &e); return; }
    }
                         ...
                         target_object_ty: ty::t)
{
    ...
    fn push_cast_obligation(fcx: &FnCtxt,
        ...
        let target_obligation = Obligation::new(cast_expr.span,
                                                target_trait_ref);
        let pending_resolution = fcx.register_obligation(target_obligation);
...
pub fn resolve_fcx_obligations_or_error(fcx: &FnCtxt) {
    ...
    let mut fulfillment_context = fcx.inh.fulfillment_context.borrow_mut();
    let r = fulfillment_context.select_all_or_error(fcx.infcx(),
                                                    &fcx.inh.param_env);
    match r {
        Ok(()) => { }
        Err(e) => { report_fulfillment_error(fcx, &e); }
    }
}
...
pub fn report_selection_error(fcx: &FnCtxt,
    ...
    match *error {
        ...
        OutputTypeParameterMismatch(ref e) => {
            let (trait_ref, self_ty) = resolve_trait_ref(fcx, obligation);
            if !ty::type_is_error(self_ty) {
                fcx.tcx().sess.span_err(
                    obligation.span,
                    format!(
                        "type mismatch for trait `{}` when \
...
pub fn try_resolve_fcx_obligations(fcx: &FnCtxt) -> bool {
    /*! Resolve as many obligations as we can at present. */

    fcx.inh.fulfillment_context
        .borrow_mut()

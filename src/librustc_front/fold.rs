...
pub trait Folder : Sized {
    ...
    fn fold_item_def(&mut self, i: ItemDef) -> ItemDef {
        noop_fold_item_def(i, self)
    }
...
pub fn noop_fold_decl<T: Folder>(d: P<Decl>, fld: &mut T) -> P<Decl> {
    d.map(|Spanned { node, span }| {
        match node {
            ...
            DeclItem(it) => Spanned {
                node: DeclItem(fld.fold_item_def(it)),
                span: fld.new_span(span),
            },
        }
    })
...
pub fn noop_fold_mod<T: Folder>(Mod { inner, items }: Mod, folder: &mut T) -> Mod {
    Mod {
        ...
        items: items.into_iter().map(|x| folder.fold_item_def(x)).collect(),
    }
}

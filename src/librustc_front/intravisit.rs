...
pub trait Visitor<'v> : Sized {
    ///////////////////////////////////////////////////////////////////////////
    // These are the points where we visit an enclosed item: we do not
    // visit the item contents in this visitor, but instead just pass
    // the node-id of the item so you can lookup its definition
    // yourself, if desired.
    //
    // Note that if you have a particular item, you can walk its
    // contents with `walk_item` (or `walk_foreign_item`,
    // `walk_trait_item`, etc).
    ...
    fn visit_closure_def(&mut self, _id: NodeId) {
    }

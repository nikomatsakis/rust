...
//!
//! Note: This walker only visits the content of a SINGLE ITEM (or
//! "ITEM-LIKE" thing, such as a trait or impl item). It does not
//! visit nested items. You should generally use this inside the
//! context of a larger visitor, such as
//! `librustc::middle::def_visitor`, that walks the items themselves
//! (and then just use this visitor to walk a particular item).
//!
...
pub trait Visitor<'v> : Sized {
    ///////////////////////////////////////////////////////////////////////////
    // Def methods. When walking the contents of an item, we will
    // sometimes encounter nested item definitions. We do not by
    // default walk the contents of these nested items, instead we
    // call these `def` routines with the node-id. The idea is that if
    // you want to see the AST of items other than the one you are
    // currently processing, you have to look it up in the AST Map
    // using the id given. This lets us track the fact that your
    // analysis, when processing an item X, also read the contents of
    // item Y (which is likely a bug).
    //
    // Anyway, you rarely want to implement or touch these methods.
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Content hooks. These methods are called by the `walk` methods
    // as we traverse each kind of HIR node. The default
    // implementation just re-invokes walk to continue the walk.
...
pub fn walk_item<'v, V: Visitor<'v>>(visitor: &mut V, item: &'v Item) {
    ...
    match item.node {
        ...
        ItemTrait(_, ref generics, ref bounds, ref items) => {
            visitor.visit_generics(generics);

...
pub fn lower_decl(lctx: &LoweringContext, d: &Decl) -> P<hir::Decl> {
    match d.node {
        ...
        DeclItem(ref it) => P(Spanned {
            node: hir::DeclItem(lower_item_def(lctx, it)),
            span: d.span,
        }),
    }
}
    ...
    P(hir::ImplItem {
        ...
        node: match i.node {
            ...
            MethodImplItem(ref sig, ref body) => {
                hir::MethodImplItem(lower_method_sig(lctx, sig), lower_block(lctx, body))
            }
...
pub fn lower_mod(lctx: &LoweringContext, m: &Mod) -> hir::Mod {
    hir::Mod {
        inner: m.inner,
        items: m.items.iter().map(|x| lower_item_def(lctx, x)).collect(),
    }
}
...
pub fn lower_item_def(_lctx: &LoweringContext, i: &Item) -> hir::ItemDef {
    hir::ItemDef { id: i.id }
}

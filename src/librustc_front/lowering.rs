...
pub fn lower_ty_binding(lctx: &mut LoweringContext, b: &TypeBinding) -> P<hir::TypeBinding> {
    P(hir::TypeBinding {
        id: b.id,
        name: b.ident.name,
    ...
    P(hir::Ty {
        ...
        node: match t.node {
            ...
            TyRptr(ref region, ref mt) => {
                hir::TyRptr(lower_opt_lifetime(lctx, region),
                            lower_mt(lctx, mt))
            }
            TyBareFn(ref f) => {
            ...
            TyParen(ref ty) => hir::TyParen(lower_ty(lctx, ty)),
...
pub fn lower_variant(lctx: &mut LoweringContext, v: &Variant) -> P<hir::Variant> {
    P(Spanned {
        node: hir::Variant_ {
...
pub fn lower_lifetimes(lctx: &mut LoweringContext, lts: &Vec<Lifetime>) -> Vec<hir::Lifetime> {
    ...
    match *vdata {
        VariantData::Struct(ref fields, id) => {
            hir::VariantData::Struct(fields.iter()
                                           .map(|f| lower_struct_field(lctx, f)).collect(), id)
        }
...
pub fn lower_struct_field(lctx: &mut LoweringContext, f: &StructField) -> hir::StructField {
    Spanned {
...
pub fn lower_opt_bounds(lctx: &mut LoweringContext,
...
pub fn lower_item_underscore(lctx: &mut LoweringContext, i: &Item_) -> hir::Item_ {
    match *i {
...
pub fn lower_trait_item(lctx: &mut LoweringContext, i: &TraitItem) -> P<hir::TraitItem> {
    P(hir::TraitItem {
        id: i.id,
        name: i.ident.name,
        attrs: i.attrs.clone(),
...
pub fn lower_impl_item(lctx: &mut LoweringContext, i: &ImplItem) -> P<hir::ImplItem> {
    P(hir::ImplItem {
        id: i.id,
        name: i.ident.name,
        attrs: i.attrs.clone(),
        ...
        node: match i.node {
            ConstImplItem(ref ty, ref expr) => {
                hir::ConstImplItem(lower_ty(lctx, ty), lower_expr(lctx, expr))
            }
            MethodImplItem(ref sig, ref body) => {
                hir::MethodImplItem(lower_method_sig(lctx, sig),
                                    lower_block(lctx, body))
            }
            TypeImplItem(ref ty) => hir::TypeImplItem(lower_ty(lctx, ty)),
...
pub fn lower_macro_def(_lctx: &mut LoweringContext, m: &MacroDef) -> hir::MacroDef {
    hir::MacroDef {
        name: m.ident.name,
        attrs: m.attrs.clone(),
        id: m.id,
...
pub fn lower_item(lctx: &mut LoweringContext, i: &Item) -> P<hir::Item> {
    P(lower_item_simple(lctx, i))
}

// fold one item into exactly one item
pub fn lower_item_simple(lctx: &mut LoweringContext, i: &Item) -> hir::Item {
...
pub fn lower_foreign_item(lctx: &mut LoweringContext, i: &ForeignItem) -> P<hir::ForeignItem> {
    P(hir::ForeignItem {
        id: i.id,
        name: i.ident.name,
        attrs: i.attrs.clone(),
        node: match i.node {
            ForeignItemFn(ref fdec, ref generics) => {
                hir::ForeignItemFn(lower_fn_decl(lctx, fdec),
                                   lower_generics(lctx, generics))
            }
...
pub fn lower_pat(lctx: &mut LoweringContext, p: &Pat) -> P<hir::Pat> {
    P(hir::Pat {
        id: p.id,
        node: match p.node {
            PatWild(k) => hir::PatWild(lower_pat_wild_kind(lctx, k)),
            ...
            PatRegion(ref inner, mutbl) => hir::PatRegion(lower_pat(lctx, inner),
                                                          lower_mutability(lctx, mutbl)),
...
fn cache_ids<'a, OP, R>(lctx: &mut LoweringContext, expr_id: NodeId, op: OP) -> R
    where OP: FnOnce(&mut LoweringContext) -> R
{
...
pub fn lower_stmt(lctx: &mut LoweringContext, s: &Stmt) -> P<hir::Stmt> {
    match s.node {
        StmtDecl(ref d, id) => {
            P(Spanned {
...
pub fn lower_capture_clause(_lctx: &mut LoweringContext, c: CaptureClause) -> hir::CaptureClause {
    match c {
...
pub fn lower_visibility(_lctx: &mut LoweringContext, v: Visibility) -> hir::Visibility {
    match v {
        Public => hir::Public,
...
pub fn lower_pat_wild_kind(_lctx: &mut LoweringContext, p: PatWildKind) -> hir::PatWildKind {
    match p {
...
pub fn lower_struct_field_kind(lctx: &mut LoweringContext,
                               s: &StructFieldKind)
                               -> hir::StructFieldKind {
    match *s {
...
fn expr_break(lctx: &mut LoweringContext, span: Span) -> P<hir::Expr> {
...
fn expr_ident(lctx: &mut LoweringContext, span: Span, id: Ident) -> P<hir::Expr> {
    expr_path(lctx, path_ident(span, id))
}

fn expr_mut_addr_of(lctx: &mut LoweringContext, span: Span, e: P<hir::Expr>) -> P<hir::Expr> {
    expr(lctx, span, hir::ExprAddrOf(hir::MutMutable, e))
}

fn expr_path(lctx: &mut LoweringContext, path: hir::Path) -> P<hir::Expr> {
...
fn expr_block(lctx: &mut LoweringContext, b: P<hir::Block>) -> P<hir::Expr> {
    expr(lctx, b.span, hir::ExprBlock(b))
}

fn expr_tuple(lctx: &mut LoweringContext, sp: Span, exprs: Vec<P<hir::Expr>>) -> P<hir::Expr> {
    expr(lctx, sp, hir::ExprTup(exprs))
}

fn expr(lctx: &mut LoweringContext, span: Span, node: hir::Expr_) -> P<hir::Expr> {
    P(hir::Expr {
        id: lctx.next_id(),
        node: node,
        span: span,
...
fn pat_ident(lctx: &mut LoweringContext, span: Span, ident: Ident) -> P<hir::Pat> {
...
fn pat(lctx: &mut LoweringContext, span: Span, pat: hir::Pat_) -> P<hir::Pat> {
    P(hir::Pat {
        id: lctx.next_id(),
        node: pat,
        span: span,
...
fn std_path(lctx: &mut LoweringContext, components: &[&str]) -> Vec<Ident> {
    let mut v = Vec::new();

...
fn mir_borrowck<'a, 'tcx>(
    ...
    if {
        !tcx.has_attr(def_id, "rustc_mir_borrowck") && !tcx.sess.opts.debugging_opts.borrowck_mir
...
impl<'cx, 'gcx, 'tcx> MirBorrowckCtxt<'cx, 'gcx, 'tcx> {
    ...
    fn append_lvalue_to_string(
        ...
        match *lvalue {
            ...
            Lvalue::Projection(ref proj) => {
                ...
                match proj.elem {
                    ProjectionElem::Deref => if autoderef {
    ...
    fn describe_field(&self, base: &Lvalue, field_index: usize) -> String {
        match *base {
            ...
            Lvalue::Static(ref static_) => self.describe_field_from_ty(&static_.ty, field_index),
            Lvalue::Projection(ref proj) => match proj.elem {
                ProjectionElem::Deref => self.describe_field(&proj.base, field_index),
    ...
    fn describe_field_from_ty(&self, ty: &ty::Ty, field_index: usize) -> String {
        if ty.is_box() {
            ...
            match ty.sty {
                ...
                ty::TyTuple(_, _) => format!("{}", field_index),

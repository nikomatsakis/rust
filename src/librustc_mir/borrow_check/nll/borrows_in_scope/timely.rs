...
pub(super) fn timely_dataflow(dump_enabled: bool, all_facts: AllFacts) -> LiveBorrowResults {
    let result = Arc::new(Mutex::new(LiveBorrowResults::new(dump_enabled)));
            ...
            worker.dataflow::<(), _, _>(|scope| {
                ...
                let (subset, requires) = scope.scoped(|subscope| {
                    ...
                    let subset = Variable::from(Collection::new(
                        None::<((RegionVid, RegionVid, LocationIndex), _, _)>
                            .into_iter()
                            .to_stream(subscope),
                    ));
                    ...
                    let requires = Variable::from(Collection::new(
                        None::<((RegionVid, BorrowIndex, LocationIndex), _, _)>
                            .into_iter()
                            .to_stream(subscope),
                    ));
                    ...
                    let dead_can_reach = {
                        let dead_can_reach = Variable::from(Collection::new(
                            None::<(((RegionVid, LocationIndex), (RegionVid, LocationIndex)), _, _)>
                                .into_iter()
                                .to_stream(subscope),
                        ));

...
impl<N: Idx> RegionValues<N> {
    ...
    crate fn add_element(
        ...
        elem.add_to_row(elements, self, r)
    }

    /// Adds all the control-flow points to the values for `r`.
    crate fn add_all_points(&mut self, elements: &RegionValueElements, r: N) {
        // FIXME OMG so inefficient. We'll fix later.
        for p in elements.all_points() {
            self.points.add(r, p);
        }
    }
    ...
    crate fn contains(
        ...
        elem.contained_in_row(elements, self, r)
    }
    ...
    crate fn locations_outlived_by<'a>(
        ...
        self.points
            .row(r)
            .into_iter()
            .flat_map(move |set| set.iter().map(move |p| elements.to_location(p)))
    }
    ...
    crate fn elements_contained_in<'a>(
        ...
        let points_iter = self
            .locations_outlived_by(elements, r)
...
impl ToElementIndex for Location {
    fn add_to_row<N: Idx>(
        ...
        let index = elements.point_from_location(self);

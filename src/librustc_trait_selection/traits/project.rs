...
fn prune_cache_value_obligations<'a, 'tcx>(
    ...
    let mut obligations: Vec<_> = result
        ...
        .filter(|obligation| {
            match obligation
                .predicate
                .ignore_qualifiers_with_unbound_vars(infcx.tcx)
                .skip_binder()
                .kind()
            {
...
fn assemble_candidates_from_predicates<'cx, 'tcx>(
    ...
    for predicate in env_predicates {
        ...
        if let &ty::PredicateKind::Projection(data) =
            predicate.ignore_qualifiers_with_unbound_vars(infcx.tcx).skip_binder().kind()
        {
...
fn confirm_object_candidate<'cx, 'tcx>(
    ...
    let env_predicate = {
        ...
        let env_predicates = env_predicates.filter_map(|o| {
            match o.predicate.ignore_qualifiers_with_unbound_vars(selcx.tcx()).skip_binder().kind()
            {
                &ty::PredicateKind::Projection(data)

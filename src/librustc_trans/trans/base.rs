...
pub fn compare_scalar_types<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    match t.sty {
        ...
        ty::ty_bool | ty::ty_uint(_) | ty::ty_char => {
            ICmp(bcx, bin_op_to_icmp_predicate(bcx.ccx(), op, false), lhs, rhs)
        }
        ty::ty_ptr(mt) if common::type_is_sized(bcx.tcx(), mt.ty) => {
            ICmp(bcx, bin_op_to_icmp_predicate(bcx.ccx(), op, false), lhs, rhs)
        }
        ty::ty_int(_) => {
            ICmp(bcx, bin_op_to_icmp_predicate(bcx.ccx(), op, true), lhs, rhs)
        }
        ty::ty_float(_) => {
            FCmp(bcx, bin_op_to_fcmp_predicate(bcx.ccx(), op), lhs, rhs)
        }
...
pub fn compare_simd_types<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    SExt(bcx, ICmp(bcx, cmp, lhs, rhs), val_ty(lhs))
}

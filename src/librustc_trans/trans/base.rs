...
use trans::collector::{self, CachedMir, TransItem, TransItemState, TransItemCollectionMode};
...
impl<'blk, 'tcx> FunctionContext<'blk, 'tcx> {
    ...
    pub fn new(ccx: &'blk CrateContext<'blk, 'tcx>,
        ...
        let mir = if let (Some(id), true) = (local_id, use_mir) {
            ccx.mir_map().map.get(&id).map(CachedMir::Ref)
        } else if let (Some(def_id), true) = (def_id, use_mir) {
            use std::collections::hash_map::Entry::*;

            // Try to get cached MIR from an external crate.
            let mut cache = ccx.mir_cache().borrow_mut();
            let mir = match cache.entry(def_id) {
                Occupied(mir) => Some(mir.get().clone()),
                Vacant(entry) => {
                    ccx.sess().cstore.maybe_get_item_mir(ccx.tcx(), def_id).map(|mir| {
                        entry.insert(Rc::new(mir)).clone()
                    })
                }
            };
            mir.map(CachedMir::Owned)
        } else {
            None
        };

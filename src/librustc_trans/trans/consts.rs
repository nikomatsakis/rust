...
pub fn const_lit(cx: &CrateContext, e: &ast::Expr, lit: &ast::Lit)
    ...
    match lit.node {
        ...
        ast::LitBinary(ref data) => {
            let g = const_addr_of(cx, C_bytes(cx, &data[]));
            let cs = ptrcast(g, Type::i8p(cx));
            assert!(cx.const_globals().borrow_mut()
                      .insert(cs as int, g).is_none());
            assert_eq!(abi::FAT_PTR_ADDR, 0);
            assert_eq!(abi::FAT_PTR_EXTRA, 1);
            C_struct(cx, &[cs, C_uint(cx, data.len())], false)
        }
    }
}
...
fn const_addr_of_mut(ccx: &CrateContext, cv: ValueRef) -> ValueRef {
    unsafe {
...
pub fn const_addr_of(ccx: &CrateContext, cv: ValueRef) -> ValueRef {
    let gv = const_addr_of_mut(ccx, cv);
    unsafe {
...
pub fn get_const_expr_as_global<'a, 'tcx>(ccx: &CrateContext<'a, 'tcx>,
    ...
    let lvalue = const_addr_of(ccx, val);
        ...
        Some(ty::AdjustDerefRef(adj)) => {
            ...
            match second_autoref {
                ...
                Some(box ty::AutoUnsize(ref k)) => {
                    ...
                    assert!(cx.const_globals().borrow_mut()
                              .insert(base as int, llconst).is_none());
                    assert_eq!(abi::FAT_PTR_ADDR, 0);
                    assert_eq!(abi::FAT_PTR_EXTRA, 1);
    ...
    unsafe {
        ...
        return match e.node {
          ...
          ast::ExprAddrOf(ast::MutImmutable, ref sub) => {
              ...
              let (e, _) = const_expr(cx, &**sub, param_substs);
              const_addr_of(cx, e)
          }
          ast::ExprAddrOf(ast::MutMutable, ref sub) => {
              let (e, _) = const_expr(cx, &**sub, param_substs);
              const_addr_of_mut(cx, e)
          }
          ast::ExprTup(ref es) => {

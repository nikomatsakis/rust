...
use rustc_middle::ty::{self, Predicate, PredicateKind, Ty, TyCtxt};
use rustc_span::Span;
...
fn ensure_drop_predicates_are_implied_by_item_defn<'tcx>(
    ...
    for (predicate, predicate_sp) in dtor_predicates.predicates {
        ...
        let predicate_matches_closure = |p: &'_ Predicate<'tcx>| {
            ...
            match (predicate, p) {
                (PredicateKind::Trait(a, _), PredicateKind::Trait(b, _)) => {
                    relator.relate(a, b).is_ok()
                }
                (PredicateKind::Projection(a), PredicateKind::Projection(b)) => {
                    relator.relate(a, b).is_ok()
                }
                _ => predicate == p,
            }

...
impl<'a,'tcx> ProbeContext<'a,'tcx> {
        ...
        self.elaborate_bounds(&bounds, |this, poly_trait_ref, item| {
            ...
            if let Some(ref m) = item.as_opt_method() {
                ...
                assert_eq!(m.generics.regions.get_slice(subst::TypeSpace).len(),
                           trait_ref.substs.regions().unwrap().get_slice(subst::TypeSpace).len());
                assert_eq!(m.generics.types.get_slice(subst::SelfSpace).len(),
                           trait_ref.substs.types.get_slice(subst::SelfSpace).len());
                assert_eq!(m.generics.regions.get_slice(subst::SelfSpace).len(),
                           trait_ref.substs.regions().unwrap().get_slice(subst::SelfSpace).len());
            }
                            ...
                            -> Ty<'tcx>
    {
        ...
        assert_eq!(substs.regions().unwrap().len(subst::FnSpace), 0);

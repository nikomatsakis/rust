...
pub fn instantiate_path<'a, 'tcx>(fcx: &FnCtxt<'a, 'tcx>,
    ...
    for &space in &[subst::SelfSpace, subst::TypeSpace, subst::FnSpace] {
        ...
        assert_eq!(substs.regions().unwrap().len(space), region_defs.len(space));
    }
    ...
    if let Some((ty::ImplContainer(impl_def_id), self_ty)) = ufcs_associated {
        ...
        assert_eq!(substs.regions().unwrap().len(subst::TypeSpace),
...

        {
            ...
            for (i, lifetime) in data.lifetimes.iter().enumerate() {
                ...
                if i < region_count {
                    ...
                    substs.mut_regions().unwrap().truncate(space, 0);
                    break;
                }
            }
        }
    ...
    fn adjust_region_parameters(
        ...
        substs: &mut Substs)
    {
        let provided_len = substs.mut_regions().unwrap().len(space);
        let desired = defs.get_slice(space);
        ...
        if provided_len == 0 {
            substs.mut_regions().unwrap().replace(
                space,

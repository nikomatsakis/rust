...
#[derive(Clone)]
pub struct LoopCtxt<'gcx: 'tcx, 'tcx> {
    may_break: bool,
...
#[derive(Clone)]
pub struct EnclosingLoops<'gcx: 'tcx, 'tcx> {
    stack: Vec<LoopCtxt<'gcx, 'tcx>>,
    by_id: NodeMap<usize>,
}

impl<'gcx, 'tcx> EnclosingLoops<'gcx, 'tcx> {
    fn find_loop(&mut self, id: ast::NodeId) -> &mut LoopCtxt<'gcx, 'tcx> {
        let ix = *self.by_id.get(&id).unwrap_or_else(|| {
            bug!("could not find enclosing loop with id {}", id);
        });
        &mut self.stack[ix]
    }
}
              ...
              if let Ok(loop_id) = label.loop_id.into() {
                  ...
                  if let Some(ref e) = *expr_opt {
                      ...
                      let opt_coerce_to = {
                          ...
                          enclosing_loops.find_loop(loop_id)
                                         .coerce
                                         .as_ref()
                  ...
                  let mut enclosing_loops = self.enclosing_loops.borrow_mut();
                  let ctxt = enclosing_loops.find_loop(loop_id);

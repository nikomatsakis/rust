...
impl<'ccx, 'tcx> CheckTypeWellFormedVisitor<'ccx, 'tcx> {
        ...
        match item.node {
            ...
            ast::ItemTrait(_, ref ast_generics, _, _) => {
                ...
                reject_non_type_param_bounds(
                    ...
                    item.span,
                    &trait_def.generics);
                self.check_variances(item, ast_generics, &trait_def.generics,
                                     self.tcx().lang_items.phantom_setter());
            }
            _ => {}
        }
    }
    ...
    {
        let item_def_id = local_def(item.id);
        let type_scheme = ty::lookup_item_type(self.tcx(), item_def_id);
    ...
    {
        let variance_lang_items = vec![
            self.tcx().lang_items.phantom_getter(),
            self.tcx().lang_items.phantom_setter(),
            self.tcx().lang_items.phantom_data(),
            self.tcx().lang_items.phantom_cell(),
        ];
        ...
        identify_constrained_type_params(self.tcx(),
                                         ty_generics.predicates.as_slice(),
                                         None,

...
impl<'cx, 'tcx> OrphanChecker<'cx, 'tcx> {
    fn check_def_id(&self, item: &ast::Item, def_id: ast::DefId) {
        if def_id.krate != ast::LOCAL_CRATE {
            ...
            // HACK(eddyb) find the closest enclosing module, for both the
            // type and the impl. This should not be necessary after UFCS.
            let enclosing_module = |mut id| loop {
                let parent = self.tcx.map.get_parent(id);
                if parent == id || parent == ast::CRATE_NODE_ID {
                    return parent;
                }
                if let ast_map::NodeItem(i) = self.tcx.map.get(parent) {
                    if let ast::ItemMod(_) = i.node {
                        return parent;
                    }
                }
                id = parent;
            };
            if enclosing_module(item.id) != enclosing_module(def_id.node) {
                span_err!(self.tcx.sess, item.span, E0123,
                          "inherent implementations are only allowed on \

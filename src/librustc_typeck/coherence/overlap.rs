                ...
                match trait_ref.self_ty().sty {
                    ty::TyTrait(ref data) => {
                        ...
                        if !traits::is_object_safe(self.tcx, data.principal_def_id()) {
                            // FIXME. This just means the self-ty is
                            // illegal; WF will report this error. But
                            // it will do so as a warning for a
                            // release or two.  For backwards compat
                            // reasons, then, we continue to report it
                            // here so that things which were errors
                            // remain errors.

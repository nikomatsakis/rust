...
fn required_predicates_to_be_wf<'tcx>(
    ...
    calculated_predicates: &FxHashMap<DefId, Rc<Vec<ty::Predicate<'tcx>>>>,
    ...
    match field_ty.sty {
        ...
        //
        // If you have A = &'a T, then mt.ty here represents T and ty represents A (and r represents 'a).
        // So we want T: 'a.
        ty::TyRef(r, mt) => {
        ...
        ty::TyAdt(def, substs) => {
            ...
            // // Get type generics(not lifetimes) of def
            // // and get lifetime generics of def
            // let (type_generics, lifetime_generics) = def
            //     .get_generics()
            //     .filter(is_type_generic);

            // // Create OutlivesPredicates for each type/lifetime.
            // // We will use this to test if the same OutlivesPredicate
            // // also exists in the preciously calculated map
            // let outlive_pred_pairs = type_generics.map( |t| =>
            //     lifetime_generics.map( |l| =>
            //         ty::OutlivesPredicates(t, l)
            // ));



            // // For each type generic see if it exists in the
            // // previously calculated predicates
            // let ty_exists_in_pred_map = type_generics
            //     .filter( |g| =>
            //         calculated_predicates.contains_key(g)
            //     );
            ...
            // ty_exists_in_pred_map.iter( |t| =>
            //     let add_pred_to_field = calculated_predicates
            //         .get_predicates(t)
            //         .intersect(outlive_pred_pairs);

            //     predicates.chain(add_pred_to_field);
            // );
